...
// Auth env type for routes that expect auth context
type AuthEnv = {
  Variables: {
    userId: string;
    email: string;
...
function buildNoteFilter(params: Record<string, unknown>): Record<string, unknown> {
  ...
  if (typeof params['tags'] === 'string' && params['tags'].length > 0) {
  ...
  if (params['createdAfter'] || params['createdBefore']) {
    ...
    if (params['createdAfter']) {
      createdAtFilter['$gte'] = new Date(params['createdAfter'] as string);
    }
    if (params['createdBefore']) {
      createdAtFilter['$lte'] = new Date(params['createdBefore'] as string);
    }
