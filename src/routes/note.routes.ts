...
import { buildNoteFilter } from '../utils/query-builder.js';
...
const listNotesRoute = createRoute({
  ...
  request: {
    query: CursorPaginationSchema.omit({ sort: true }).extend({
      sort: z.enum(['createdAt', 'updatedAt', 'title', 'isPinned']).default('createdAt'),
    }).extend(NoteQuerySchema.shape),
  },
  responses: {
    200: {
...
noteRouter.openapi(updateNoteRoute, async (c) => {
  ...
  if (userRole !== 'admin') {
    ...
    const existingNote = await noteService.findById(id, userId);
