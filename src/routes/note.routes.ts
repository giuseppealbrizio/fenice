...
import type { Note, NoteQuery } from '../schemas/note.schema.js';
...
function buildNoteFilter(params: NoteQuery, currentUserId?: string): Record<string, unknown> {
  const filter: Record<string, unknown> = {};

  // If no specific userId filter, show user's own notes + public notes
  if (!params.userId) {
    if (currentUserId) {
      filter['$or'] = [
      ...
      // No authenticated user - only public notes
      filter['isPublic'] = true;
    }
  } else {
    // Specific user requested
    if (params.userId === currentUserId) {
      // Own notes - no additional filter needed
      filter['userId'] = params.userId;
    } else {
      // Other user's notes - only public ones
      filter['userId'] = params.userId;
      filter['isPublic'] = true;
    }
  }
  ...
  if (params.tags) {
    const tagList = params.tags.split(',').map(tag => tag.trim()).filter(Boolean);
    if (tagList.length > 0) {
  ...
  if (params.isPublic !== undefined) {
    filter['isPublic'] = params.isPublic;
  }
