...
function buildNoteFilter(params: Record<string, unknown>): Record<string, unknown> {
  ...
  if (params['tags']) {
    const tagArray = String(params['tags']).split(',').map((tag) => tag.trim());
    filter['tags'] = { $in: tagArray };
  }

  if (params['createdAfter']) {
    const createdAtFilter = filter['createdAt'] as Record<string, unknown> || {};
    createdAtFilter['$gte'] = new Date(String(params['createdAfter']));
    filter['createdAt'] = createdAtFilter;
  }

  if (params['createdBefore']) {
    const createdAtFilter = filter['createdAt'] as Record<string, unknown> || {};
    createdAtFilter['$lte'] = new Date(String(params['createdBefore']));
    filter['createdAt'] = createdAtFilter;
  }
