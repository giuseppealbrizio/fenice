...
function buildNoteFilter(
  params: z.infer<typeof NoteQuerySchema>,
  userId?: string
): Record<string, unknown> {
  const filter: Record<string, unknown> = {};

  // If userId is provided, filter by user's notes or public notes
  if (userId) {
    filter['$or'] = [{ userId }, { isPublic: true }];
  }
  ...
  if (params.search) {
    ...
    if (filter['$and']) {
      (filter['$and'] as Record<string, unknown>[]).push(searchFilter);
    } else {
  ...
  if (params.tags) {
    ...
    const tagFilter = { tags: { $in: tagArray } };

    if (filter['$and']) {
      (filter['$and'] as Record<string, unknown>[]).push(tagFilter);
    } else {
  ...
  if (params.isPublic !== undefined) {
    const publicFilter = { isPublic: params.isPublic };

    if (filter['$and']) {
      (filter['$and'] as Record<string, unknown>[]).push(publicFilter);
    } else {
  ...
  if (params.createdAfter || params.createdBefore) {
    ...
    const createdAtFilter = { createdAt: dateFilter };

    if (filter['$and']) {
      (filter['$and'] as Record<string, unknown>[]).push(createdAtFilter);
    } else {
      filter['$and'] = [createdAtFilter];
    }
  }
...
noteRouter.openapi(listNotesRoute, async (c) => {
  ...
  const filter = buildNoteFilter(filterParams, userId);
