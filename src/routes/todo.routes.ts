...
function serializeTodo(todo: TodoDocument): Todo {
  ...
  return {
    ...
    dueDate: json['dueDate'] ? json['dueDate'] as string : undefined,
    completedAt: json['completedAt'] ? json['completedAt'] as string : undefined,
...
todoRouter.openapi(listTodosRoute, async (c) => {
  ...
  const filter = buildTodoFilter(filterParams);
  const result = await todoService.findAll(userId, filter, { cursor, limit, sort, order });
  return c.json(
    {
...
todoRouter.openapi(updateTodoRoute, async (c) => {
  ...
  const { id } = c.req.valid('param');
  const body = c.req.valid('json');
  const todo = await todoService.update(id, userId, body);
  return c.json(serializeTodo(todo), 200);
});
